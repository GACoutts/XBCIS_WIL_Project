...
// Serve quote documents (PDF/DOC/DOCX) with proper content type
app.use('/uploads/quotes', express.static(path.resolve(__dirname, 'uploads/quotes'), {
  dotfiles: 'deny',
  index: false,
  maxAge: '1d',
  // No need to force PDF only; Express sets Content-Type from extension.
}));
...
app.use('/api/tickets', appointmentsRouter)
